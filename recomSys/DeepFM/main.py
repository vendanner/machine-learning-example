...
特征工程：
    一: 删除无效一些无效特征
    二：新增数据缺失特征和 ['ps_car_13_x_ps_reg_03']
    三：category 特征扩展成多维特征（类似 one-hot，但不同的是one-hot 长度不是max，而是len() ）
模型架构：
    Deep：
...
def load_data():
    ...
    # 构造新特征 'ps_car_13_x_ps_reg_03' 和统计丢失的特征数
...
def run_base_model_dfm(dfTrain,dfTest,folds,dfm_params):
    ...
    print("Xi_train[0]: ",Xi_train[0])
    print("Xv_train[0]: ",Xv_train[0])
    print("type:")
    ...
    # 函数式，传入(x,l)；供交叉验证的训练集(数据集被打散)取特征
...
# 类似 k 折交叉验证(此处为 3 折)，但 StratifiedKFold 保证了数据分布(测试集不会都是在一起的)
