...
特征工程：
    1、标签值one-hot
    2、连续值按值所在区间的index 来one-hot
    3、(age,capital-gain),("capital-gain", "capital-loss")组合特征
"""
...
def train_data(train_file_path,test_file_path,out_train_path,out_test_path,features_path):
    ...
    for feature_str in dis_feature_list:
        ...
        dis_feature_num += tmp
        feature_num_dict[feature_str] = tmp
    # 连续值特征变换
    for feature_str in con_feature_list:
        tmp = process_con_feature(feature_str,train_df,test_df)
        con_feature_num += tmp
        feature_num_dict[feature_str] = tmp

    # 笛卡儿积组合特征
    new_feature_len = combine_feature("age", "capital-gain", "age_gain", train_df, test_df, feature_num_dict)
    new_feature_len_two = combine_feature("capital-gain", "capital-loss", "loss_gain", train_df, test_df, feature_num_dict)
    train_df = train_df.reindex(columns=index_list+["age_gain","loss_gain","label"])
    test_df = test_df.reindex(columns=index_list+["age_gain","loss_gain","label"])
    print(feature_num_dict)
    util.save_df_data(train_df,out_train_path)
    util.save_df_data(test_df, out_test_path)
    util.save_data("feature_num="+str(dis_feature_num+con_feature_num+new_feature_len+new_feature_len_two),features_path)
